...
3. **Infraestructura.** *Actions > infra-deploy > Run workflow* (o `gh workflow run infra-deploy.yml`). Tarda alrededor de un minuto. El resumen del job muestra `api_endpoint`, `frontend_url` y la URI de redireccion para Entra ID. Sin tenant todavia se puede ejecutar igual: se usan marcadores de issuer/audience y el BFF no arranca hasta configurarlos.
...
- Cambiar `jwt_issuer`/`jwt_audience` recrea la instancia (`user_data_replace_on_change`); la IP elastica y los artefactos en S3 hacen que vuelva sola al servicio.
